...
export async function handler(event: any) {
  ...
  const publishCommand = new PublishCommand({
    topic: 'integ/0x00158d0005829a78/attributeReport/1/msTemperatureMeasurement',
  ...
  log.info('publishResponse', { httpStatusCode: publishResponse.$metadata.httpStatusCode });

  // wait until message is processed
  await new Promise(f => setTimeout(f, 3000));

  const client = new DynamoDBClient({});
  const send = await client.send(new ScanCommand({
