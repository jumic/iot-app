...
export const handler: AWSLambda.SQSHandler = async(event: AWSLambda.SQSEvent) => {
  ...
  for (const record of event.Records) {
    ...
    if (topicParts[4] === 'msTemperatureMeasurement') {
      ...
      await MetricService.createMetric(new Metric(deviceId, new Date().toISOString(), type, value));
    }
